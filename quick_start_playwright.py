#!/usr/bin/env python3
"""
Quick-Start Example: LinkedIn Automation (Playwright Version)
Use this to test the script with safe defaults before running at scale
"""

import asyncio
from linkedin_automation_playwright import LinkedInAutomation
import os
...
async def test_mode():
    ...
    - Single company
    - No outreach (collect contacts only)
    - Headless off (see what's happening)
    - Low rate limit
    """
    print("\n" + "="*70)
    print("LinkedIn Automation - TEST MODE (Playwright)")
    print("="*70)
    print("\nSettings:")
    print("  • Companies: 1 (test)")
    print("  • Send Messages: No (collect contacts only)")
    ...
    print("  6. Export contact list to CSV + Google Sheets")
    print("\n" + "="*70 + "\n")
...
async def production_mode():
    """
    PRODUCTION MODE: Gather contacts and prepare outreach
    - Multiple companies
    - No messages by default
    - Set SEND_MESSAGES=true to enable outreach later
    - Configurable from .env
    """
    print("\n" + "="*70)
    print("LinkedIn Automation - PRODUCTION MODE (Playwright)")
    print("="*70)
...
async def resume_mode():
    ...
    """
    print("\n" + "="*70)
    print("LinkedIn Automation - RESUME MODE (Playwright)")
    print("="*70)
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        print("\n" + "="*70)
        print("LinkedIn Automation - Quick Start (Playwright)")
        print("="*70)
        print("\nUsage: python quick_start_playwright.py [mode]")
        print("\nAvailable modes:")
        ...
        print("\nExample:")
        print("  python quick_start_playwright.py test")
        print("\n" + "="*70 + "\n")
        sys.exit(1)
